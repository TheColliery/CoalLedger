...
export const CONFIG_SCHEMA = [
  ...
  { key: 'severityFloor', type: 'enum', values: ['low', 'medium', 'high', 'critical'], def: 'low', help: 'Report findings at or above this severity (severity is assigned by context in the report step, never mechanically). NOTE: like quickVsFull, this key is read by the agent from the project file directly, so the global-vs-project consent clamp in config-load.mjs does NOT cover it — a cloned repo setting a high project floor can narrow what a run reports; /coalledger:stats surfaces the effective floor so it is inspectable rather than invisible' },
  { key: 'quickVsFull', type: 'enum', values: ['quick', 'full'], def: 'quick', help: 'Default scan tier for mixed canaries: quick = mechanical layer only (~free, report-only) · full = adds the semantic layer (paid; always a separate consent). NOTE: this key is read by the agent from the project file directly, so the global-vs-project consent clamp in config-load.mjs does NOT cover it — a project setting full is honoured, and the separate per-run consent is what gates the spend' },
