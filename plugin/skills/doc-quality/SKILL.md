...
2. **Quick (mechanics):** deterministic checks per the table—a mechanics finding is CONFIRMED only when it is objectively wrong in the doc's language (a decomposed character, a doubled space), not a stylistic preference.
   ...
   - **2c. Script-mechanics (`lang-mechanics.mjs`):** same skill folder, same `cd` requirement, absolute paths. Run: `cd "<skill base dir>" && node ./lib/lang-mechanics.mjs --json <absolute-file.md> [more absolute paths ...]`. Detection is by SCRIPT RANGE found in the text, resolved per LINE, never by the doc's declared language—today ships ZH only (`zh-halfwidth-punct`, `zh-space-before-punct`; authority GB/T 15834-2011), so a non-ZH doc reports nothing here until its script's rule table ships, and a Han line carrying any Hangul or Kana is silent too (the script resolves to KO/JA on that line, vetoing the ZH rules to close a real Korean-Hanja false positive). Same exit contract as 2b (exit 0 on findings, exit 1 only on an unreadable file—never the exit code). Each `--json` entry names its rule id and `authority`; a rule with no named authority does not ship, so every hit traces to a cited standard, never a house guess. A 4-space-indented code block—and any nested list item or list-continuation paragraph indented to that depth—is skipped whole too, the same miss-never-a-hit trade `emdash.mjs` accepts.
...
|---|---|---|---|
| read | scan docs for mechanics + quality, against any house rules | `Read`·`Grep`·`Glob` (·`Bash`—the `emDash` and script-mechanics engines run via `node ./lib/emdash.mjs` / `./lib/lang-mechanics.mjs`, per Method steps 2b/2c; `Read`/`Grep`/`Glob` cannot execute either) | refuse that file, report it unscanned—never a false clean bill |
