...
- **It runs inside your agent.** No CoalLedger servers and no CoalLedger account — the code executes under your own AI agent, subject to your platform's own permission gate.
...
- **Local files only.** All state lives in files you can read: the self-update throttle stamp `~/.claude/.coalledger-update-check` (a timestamp, nothing more); the config (`~/.claude/.coalledger.json` global, optional per-project `.coalledger.json`); the docs-drift session state `os.tmpdir()/coalledger-<session-id>.docs` and `.docmemmoved` (a path list and a 0-byte marker, cleared as the session proceeds and OS-tmp reaped otherwise); and, on Antigravity, the once-per-session marker `os.tmpdir()/coalledger/ag-conductor-*.marker`. The engine scripts write nothing at all. Fix-mode checkpoints (a git stash/commit, or a copy-aside) are made by your agent in your project, on your disk.
