...
export const CONFIG_SCHEMA = [
  ...
  { key: 'severityFloor', type: 'enum', values: ['low', 'medium', 'high', 'critical'], def: 'low', help: "Report findings at or above this severity (severity is assigned by context in the report step, never mechanically). Fully settable per project, same as quickVsFull below (this key is read by the agent from the project file directly, so the global-vs-project consent clamp in config-load.mjs has no path to it): a cloned repo setting a high project floor is honoured as written and can narrow what a run reports. Unlike quickVsFull's spend, which is backstopped unconditionally at run-time (a separate per-run consent gates it regardless of this file), a narrowed report is inspectable only ON REQUEST via /coalledger:stats -- not visible by default, and stats shows the floor's value, never the count of findings it withheld" },
  { key: 'quickVsFull', type: 'enum', values: ['quick', 'full'], def: 'quick', help: 'Default scan tier for mixed canaries: quick = mechanical layer only (~free, report-only) · full = adds the semantic layer (paid; always a separate consent). Fully settable per project: this key is read by the agent from the project file directly, so the global-vs-project consent clamp in config-load.mjs has no path to it and cannot second-guess it — a project setting full is honoured as written, and the separate per-run consent (not this clamp) is what gates the actual spend' },
  ...
  { key: 'emDash', type: 'enum', values: ['unspaced', 'spaced', 'off'], def: 'off', help: "doc-quality's em-dash typography rule (CWK-073). unspaced = the house form word—word, so a SPACED em-dash is the finding; spaced = the inverse; off (factory default) = the rule does not run at all -- a house choice, never forced on another user's repo" },
  ...
  { key: 'scanEverything', type: 'bool', def: false, help: 'CWK-057, the owner\'s antivirus-scope law, one flock one colour with CoalMine\'s key of the same name/polarity/default: when true, report findings down to severityFloor "low" for this run regardless of the configured floor (default: off). Positive polarity — true = MORE disclosure, never a double negative. Bounded, not "scan everything": CoalLedger has no automatic scan-scope cut to bypass at all (every canary is agent-invoked, never auto-batch-scanned the way CoalMine\'s touch/stop pipeline is) — this key reaches ONLY severityFloor. It does NOT re-enable a canary disabled via disabledCanaries/docLeak/coalledgerMode (those are ON/OFF consent switches, not scan scope), and does NOT force quickVsFull to "full" (that would escalate PAID spend via a scope key, which hooks-safety.md §9 forbids regardless of polarity). REACH, stated where a reader meets the key (CWK-057 INSPECT MED-1, CWK-064 INSPECT MED-1): the agent now reads it as a global+project merge (project wins when set), never the project file alone — but this is still an instruction the AGENT follows from SKILL.md prose, not a gate the code enforces, so the consent clamp in config-load.mjs still has no path to it, the same way severityFloor is' },
];
