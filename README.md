...
**Docs:** [thecolliery.gitbook.io/thecolliery-docs/tools/coalledger](https://thecolliery.gitbook.io/thecolliery-docs/tools/coalledger) *(publishing soon)*

**Part of [TheColliery](https://github.com/TheColliery)**—siblings: **[CoalMine](https://github.com/TheColliery/CoalMine)** (quality canaries) · **[CoalTipple](https://github.com/TheColliery/CoalTipple)** (model/effort routing) · **[CoalBoard](https://github.com/TheColliery/CoalBoard)** (consensus board) · **[CoalHearth](https://github.com/TheColliery/CoalHearth)** (warm-resume) · **[CoalFace](https://github.com/TheColliery/CoalFace)** (fan-out discipline) · **[CoalWash](https://github.com/TheColliery/CoalWash)** (memory defrag) · **[CoalGob](https://github.com/TheColliery/CoalGob)** (OS-trash delete guard, PUBLIC BETA v0.1.0-beta.1).
...
Every tool in the series supports two config levels—a global `~/.claude/.coalledger.json` and a per-project config, resolved in this order (first found wins): `.claude/coal/coalledger.json` (the running agent's own dir—the config loader takes no agent-identity signal, so this always collapses onto `.claude` regardless of whether Claude Code or the ported Antigravity adapter is running) → other known agent dirs, `.agents/coal/coalledger.json` → `.gemini/coal/coalledger.json` → two **DEPRECATED** legacy paths, in this order: `.claude/.coalledger.json` (a per-project file, not the global `~/.claude/.coalledger.json` above, which is unchanged), then a root `.coalledger.json` (the pre-2026-08-08 shape)—both still read, no breakage—so a globally-installed skill can be tuned or **shut off per project** (`coalledgerMode: "off"` is the off-switch; a project can also disable single canaries or raise the severity floor instead)—a skill you don't need in a given project stops loading (and burning tokens) there. Write with `node scripts/configure.mjs <flags>` (the project config) or `--global <flags>` (the global layer); a write lands back wherever the config was found, except a config sitting at EITHER legacy path migrates to the project's own agent-dir home on that write (the old file removed only after the new one is written). A project with no config anywhere yet gets the first agent dir it **already has** on disk (`.claude` → `.agents` → `.gemini`)—never a bare `.claude` planted ahead of an `.agents`/`.gemini` dir that already exists; a project with NONE of the three still gets `.claude`, the same default it got before this fix. Nothing is ever auto-moved on a mere read.

**Deprecation notice.** Both legacy paths above are **DEPRECATED**; the replacement is the canonical `.claude/coal/coalledger.json`. Move the file there, or run `node scripts/configure.mjs <flags>` from a repo checkout, which makes that move on its next write (a hook never migrates). They are deprecated as of the release named by the `### Deprecated` entry in [CHANGELOG.md](CHANGELOG.md) and removable no earlier than the next MAJOR—deprecated at a MINOR, removable at a MAJOR, this series' SemVer boundary and never a calendar window. CoalLedger owns the migration. **No runtime deprecation warning is coming:** a hook may print only on its sanctioned channels (Phoenix #13), so this note and the CHANGELOG are where the change is announced. What you will see at runtime is a path report, not a warning: when the config in force was read from a legacy path, the session-start conductor adds one line of context—`[CoalLedger] LEGACY: <path> is a deprecated config path (still read); move it to .claude/coal/coalledger.json`—and a config file sitting at a path nothing reads (for example `.agents/.coalledger.json`, or `coalledger.json` with the leading dot dropped) is reported the same way—`[CoalLedger] IGNORED: <path> is not a config path; canonical = .claude/coal/coalledger.json`—because its settings have no effect. That probe checks a fixed short list of such paths in your project root for a FILE and nothing else (a directory that happens to carry one of those names is not a config, and is not reported); a config anywhere outside it is not reported.

The ceiling on those two lines: with `coalledgerMode: "off"` or the conductor disabled (`disabledCanaries`: `"conductor"` or `"all"`) these lines stay silent, a consent gate that stays; run `/coalledger:stats` instead, which reports the config file in force and any such stray file.
...
**Reads** the docs you name (plus the files their links point at, to verify those links) and your `.coalledger.json`—at whichever [config path](#-configure) holds it. The session-start conductor also checks whether a file exists at a fixed short list of near-miss config paths in your project root (existence only, never their contents) so it can name one that nothing reads. **The installed skill writes only its own scratch:** two session files in your temp dir, a self-update date stamp under `~/.claude/`, and on Antigravity a once-per-session marker—this is everything the canaries themselves ever touch, and none of it is a doc or a config file. **Runs up to three things locally:** the bundled zero-dependency AST engine (`node ./lib/md-checks.mjs`, read-only), a `git stash`/commit checkpoint before any fix (a file copy where git is absent), and—only if you consent—a documented example a doc claims works.
